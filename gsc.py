...
def gsc_build(args):
    ...
    distro = distro.split(":")
    env.loader =jinja2.FileSystemLoader(f'templates/{distro[0]}')
    ...
    build_template = env.get_template(f'Dockerfile.{distro[0]}.build.template')
...
def gsc_build_gramine(args):
    ...
    distro = distro.split(":")
    env.loader = jinja2.FileSystemLoader(f'templates/{distro[0]}')
    compile_template = env.get_template(f'Dockerfile.{distro[0]}.compile.template')
...
def gsc_sign_image(args):
    ...
    distro = distro.split(":")
    env.loader = jinja2.FileSystemLoader(f'templates/{distro[0]}')
    sign_template = env.get_template(f'Dockerfile.{distro[0]}.sign.template')
